...
export const SearchBarForHome = () => {
  ...
  // const handleOnClick = useHandleGotoDetails;

  return (
    <Popover>
      <PopoverTrigger asChild>
        <Input type="text" placeholder="Search..." className="w-[379px]" onChange={handleOnChange} value={input} />
      </PopoverTrigger>
      {input && (
          ...
          <div className="grid gap-4">
            <div className="space-y-2">
              {data?.results?.slice(0, 7).map((movie: { poster_path: string; id: string; title: string; vote_average: number }) => {
                ...
                return (
                  <div key={movie.id} className="flex flex-col gap-2" onClick={useHandleGotoDetails(movie.id)}>
