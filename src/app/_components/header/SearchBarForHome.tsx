...
export const SearchBarForHome = () => {
  ...
  console.log("gagafg", input);

  // console.log("useinput", data);
            ...
            {data?.results?.slice(0, 7).map((movie: { poster_path: string; id: string; title: string; vote_average: number }) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="w-[90%] border border-gray-300 mt-4"></div>
                </div>
              );
            })}
            <p className="hover:bg-gray-200 cursor-pointer rounded-lg p-2" onClick={() => handleNavigateSearchResult(input)}>
